...
const hasMWS = (app, module = "none") =>
  app._router.stack.filter((mws) => mws.name === module).length > 0;
...
const registerController = (UP) => (name, controller) => {
  ...
  if (name in UP.controllers)
    throw Error(`Controller ${name} already register`);
...
const universalPattern = (app = express(), options = {}) => {
  ...
  if (!hasMWS(app, "compression") && localOptions.compress)
    app.use(compression({ level: 9 }));
  if (!hasMWS(app, "cors") && localOptions.cors) app.use(cors());

  return new Promise((resolve) => {
    const yamlContent = [
      ...fs.readdirSync(localOptions.swagger.folder).map((file) => {
        if (file.split(".").pop() === "yaml") {
          return fs
            .readFileSync(path.join(localOptions.swagger.folder, file), "utf8")
            .toString();
        }
        return false;
      }),
      fs
        .readFileSync(path.join(__dirname, "./swagger", "index.yaml"), "utf8")
        .toString(),
    ]
    ...
    app.use(`${localOptions.swagger.baseDoc}/docs/`, (req, res) => {
      const content = fs
        .readFileSync(`${__dirname}/libs/swagger-ui/index.tmp`)
        .toString();
      res.end(
        content.replace("[[URL]]", `${localOptions.swagger.baseDoc}/api-docs`)
      );
    });
    app.use(`${localOptions.swagger.baseDoc}/api-docs`, (req, res) =>
      res.json(UP.swagger)
    );
